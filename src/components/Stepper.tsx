...
export default function Stepper({
  ...
  const stepsArray = Children.toArray(children as React.ReactNode[]);
  const totalSteps = stepsArray.length;
  const isCompleted = currentStep > totalSteps;
  const isLastStep = currentStep === totalSteps;
  const canProceed = !isLastStep && (isStepValid ? isStepValid(currentStep) : true);
  ...
  const handleNext = () => {
    if (!isLastStep && (isStepValid ? isStepValid(currentStep) : true)) {
      setDirection(1);
      updateStep(currentStep + 1);
    }
  };
  ...
  // External completion signal (trigger only on change after mount)
  const prevCompleteRef = useRef<number | undefined>(completeSignal);
  useEffect(() => {
    if (prevCompleteRef.current !== completeSignal) {
      if (typeof completeSignal === 'number') {
        handleComplete();
      }
      prevCompleteRef.current = completeSignal;
    }
      ...
      <div className={`step-circle-container ${stepCircleContainerClassName}`}>
        ...
        {isCompleted ? (
          <div className={`step-content-default ${contentClassName}`}>
            {renderCompleted ? renderCompleted() : null}
          </div>
        ) : (
          <>
            <StepContentWrapper
...
export function Step({ children }: React.PropsWithChildren) {
    ...
    <motion.div onClick={handleClick} className="step-indicator" animate={status} initial={false}>
      <motion.div
        variants={{
          ...
          active: { scale: 1, backgroundColor: '#a43434', color: '#a43434' },
          complete: { scale: 1, backgroundColor: '#a43434', color: '#a43434' },
        }}
...
function StepConnector({ isComplete }: { isComplete: boolean }) {
  const lineVariants = {
    ...
    complete: { width: '100%', backgroundColor: '#a43434' },
  } as const;
